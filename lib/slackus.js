...
var Slackus = function(config) {
  ...
  this.slackwebhooks = config.slack.webhook.split(',');
  this.slacks = [];
  this.slackwebhooks.foreach(function(webhook) {
    this.slacks.push(new Slack(webhook));
  }, this)
...
Slackus.prototype.sendMessage = function(message, forum) {
  ...
  this.slacks.forEach(function(slack) {
    slack.send(options, function(error) {
      if (error) {
